-- Fix candidates table to allow longer values - SIMPLIFIED VERSION
-- Run each ALTER statement separately if the full script doesn't work
...
-- Step 2: Alter columns (run these one at a time if needed)
ALTER TABLE candidates ALTER COLUMN image TYPE VARCHAR(2000);
ALTER TABLE candidates ALTER COLUMN name TYPE VARCHAR(500);
ALTER TABLE candidates ALTER COLUMN party TYPE VARCHAR(500);
